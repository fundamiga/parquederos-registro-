...
export default function Abonos() {
  ...
  const obtenerEtiquetaTipo = (a) => {
    const duracionDias = Math.round((new Date(a.fecha_fin) - new Date(a.fecha_inicio)) / (1000 * 60 * 60 * 24));
    if (a.tipo === 'semanal' || (a.observaciones && a.observaciones.includes('SEMANAL')) || a.monto === 3500 || (duracionDias > 0 && duracionDias <= 8)) {
      return {
        texto: '🟣 Semanal (7 días)',
        badgeClass: 'bg-purple-500/15 text-purple-700 dark:text-purple-300 border-purple-500/30'
      };
    }
    if (a.tipo === 'mensual' || a.monto === 14000 || duracionDias > 20) {
      return {
        texto: '📅 Mensual (30 días)',
        badgeClass: 'bg-emerald-500/15 text-emerald-700 dark:text-emerald-300 border-emerald-500/30'
      };
    }
    return {
      texto: '🗓️ Quincenal (15 días)',
      badgeClass: 'bg-emerald-500/15 text-emerald-700 dark:text-emerald-300 border-emerald-500/30'
    };
  };
  ...
  return (
    <div className="max-w-7xl mx-auto px-3.5 sm:px-6 lg:px-8 py-5 sm:py-8 pb-28 md:pb-10 flex flex-col gap-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-0.5">
            Control y edición de semanas ($3.500), quincenas ($7.000), mensualidades ($14.000) y tarifas excepcionales.
          </p>
        </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <div className="flex-1">
          <select
            ...
            className="input text-xs sm:text-sm font-bold w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800"
          >
            <option value="">🔍 Todas las Motos / Placas</option>
            {motos.map((m) => (
        ...
        {/* State Filter Pills */}
        <div className="flex items-center gap-1.5 bg-slate-100 dark:bg-slate-900/80 p-1.5 rounded-2xl border border-slate-200 dark:border-slate-800 self-start sm:self-auto">
          <button
      ...
      {/* List of Abonos */}
      {cargando ? (
        <div className="card text-center py-16 text-slate-400 font-bold text-sm bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800">
          Cargando abonos...
        </div>
        ...
        <div className="card text-center py-16 flex flex-col items-center justify-center gap-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800">
          ...
          <div>
            <h3 className="font-bold text-slate-800 dark:text-slate-200 text-base">No hay abonos registrados</h3>
            <p className="text-xs text-slate-500 mt-1 max-w-sm">
              {filtroMoto || filtroEstado !== 'todos'
                ? 'No se encontraron abonos con los filtros seleccionados.'
                : 'Registra quincenas o mensualidades para los trabajadores fijos.'}
            </p>
          </div>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {abonosFiltrados.map((a) => {
            const vencido = isPast(new Date(a.fecha_fin + 'T23:59:59'));
            const tipoInfo = obtenerEtiquetaTipo(a);
            return (
              <div
                key={a.id}
                ...
                <div className="flex flex-col gap-2 text-xs">
                  ...
                  <div className="bg-white dark:bg-slate-950/70 p-3 rounded-2xl border border-slate-200 dark:border-slate-800 flex items-center justify-between mt-1">
                    <div>
                      ...
                      <span className="font-extrabold text-slate-800 dark:text-slate-200">{tipoInfo.texto}</span>
                    </div>
            ...
            <form onSubmit={handleGuardarEdicion} className="flex flex-col gap-4 text-xs">
              ...
              <div>
                ...
                <div className="flex gap-1.5 mt-1.5">
                  <button
                    type="button"
                    onClick={() => setEditForm((f) => ({ ...f, monto: '3500', tipo: 'semanal' }))}
                    className="px-2 py-1 bg-purple-50 dark:bg-purple-950/40 border border-purple-200 dark:border-purple-500/30 rounded-lg text-[10px] font-bold text-purple-700 dark:text-purple-300"
                  >
                    Semanal ($3.500)
                  </button>
                  <button
